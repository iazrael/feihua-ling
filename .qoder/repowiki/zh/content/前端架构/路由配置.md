...
- routes 数组中“/”与“/about”的两种路由定义方式
- HomeView 的直接引入与 AboutView 的懒加载（代码分割）差异及性能优势
...
A --> C
B --> |"渲染"| F["RouterView"]
C --> |"history: createWebHistory(BASE_URL)"| G["浏览器 History API"]
C --> |"routes: '/' 与 '/about'"| H["路由表"]
H --> |"路径 '/' 组件"| D
H --> |"路径 '/about' 组件"| E
```
...
- 路由表：routes 数组定义路径、名称与组件映射；其中“/”为直引组件，“/about”为动态导入组件。
- 视图组件：HomeView 与 AboutView 分别作为首页与关于页的承载组件。
...
### 路由表：routes 数组中的“/”与“/about”
- “/”路由：使用直接引入的方式绑定 HomeView 组件，属于同步加载，首屏即可获得该组件资源，有利于首屏渲染性能。
- “/about”路由：使用动态导入（路由级代码分割）的方式绑定 AboutView 组件，仅在访问该路径时按需加载，减少初始包体积，提升首屏加载速度。
...
- 动态导入（AboutView）：构建时生成独立的代码块，仅在访问“/about”时下载，显著降低首屏包体积，改善首屏加载时间；但首次访问会有一次网络请求与解析开销。
...
## 结论
本项目通过 createRouter 与 createWebHistory 构建了清晰的前端路由体系，routes 数组以“/”与“/about”两条路径分别演示了直引与懒加载两种策略。BASE_URL 的合理使用确保了子路径部署场景下的导航一致性。整体设计兼顾了首屏性能与后续页面的按需加载，符合现代前端应用的最佳实践。
