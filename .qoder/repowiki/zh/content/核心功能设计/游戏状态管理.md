...
## 引言
本文档围绕前端游戏“飞花令”的状态机实现展开，重点阐述如何通过 Pinia store（game.ts）对游戏状态进行集中管理。基于 `useGameStore` 中定义的响应式状态（如 isPlaying、currentRound、history 等），解释游戏的三种状态（未开始、进行中、结束）如何通过布尔标志、计数器与数组数据进行管理；并分析 startGame、verifyUserSentence、aiTurn 三个核心 action 如何驱动状态转换；同时结合 history 数组的 push 操作说明交互历史的动态维护。**本次更新重点补充了新增的超时统计、模糊匹配计数、平均响应时间、最快响应、完美回合等统计字段，以及倒计时相关状态和方法的实现细节**。最后阐述 UI 组件如何从 store 获取数据以反映当前游戏状态（如回合信息、剩余机会、统计信息等）。文末更新了状态转换图以准确反映当前流程。
...
Timer["components/TimerDisplay.vue"]
Main["main.ts"]
end
App --> Router
Router --> Home
...
- [GameView.vue](file://frontend/src/views/GameView.vue#L1-L194)
- [ResultView.vue](file://frontend/src/views/ResultView.vue#L1-L190)
- [game.ts](file://frontend/src/stores/game.ts#L1-L325)
- [TimerDisplay.vue](file://frontend/src/components/TimerDisplay.vue#L1-L152)

**章节来源**
- [README.md](file://frontend/README.md#L1-L55)
- [GEMINI.md](file://GEMINI.md#L1-L21)
- [IMPLEMENTATION_SUMMARY.md](file://IMPLEMENTATION_SUMMARY.md#L57-L100)
...
- game.ts：游戏状态的 Pinia store，集中管理所有游戏状态，包含以下关键状态：
  ...
  - stats：游戏统计数据（答对、答错、提示次数、开始时间、**超时次数、模糊匹配次数、平均响应时间、最快响应、完美回合**）
  - currentHintLevel：当前提示级别
  - **timeRemaining**：当前倒计时剩余时间
  - **timerActive**：倒计时是否激活
  - **roundStartTime**：当前回合开始时间
  - **timerDuration**：倒计时时长（可配置）
- HomeView.vue：游戏起始界面，用户选择关键字后调用 store 的 startGame action。
- GameView.vue：游戏主界面，从 store 获取状态并渲染，调用 store 的 verifyUserSentence、getHint、skipRound 等 action，并集成 TimerDisplay 组件。
- ResultView.vue：游戏结果界面，从 store 获取最终状态和统计数据进行展示。
- **TimerDisplay.vue**：独立的倒计时显示组件，负责渲染倒计时UI、播放音效，并在超时时触发 handleTimeout 事件。

**章节来源**
- [game.ts](file://frontend/src/stores/game.ts#L7-L35)
- [HomeView.vue](file://frontend/src/views/HomeView.vue#L12-L24)
- [GameView.vue](file://frontend/src/views/GameView.vue#L10-L194)
- [ResultView.vue](file://frontend/src/views/ResultView.vue#L7-L190)
- [TimerDisplay.vue](file://frontend/src/components/TimerDisplay.vue#L1-L152)

## 架构总览
前端通过 Pinia store (game.ts) 集中管理游戏状态。UI 组件（HomeView, GameView, ResultView）通过 `useGameStore()` 注入 store 实例，订阅状态变化并调用 action。后端提供三类 API，分别对应“开始游戏”“验证诗句”“AI 回合”“获取提示”。UI 层根据 store 状态自动更新，从而完成状态转换。**新增的倒计时功能由 TimerDisplay 组件驱动，通过监听 store 的 timerActive 状态来启动/停止倒计时，并在超时时通知 GameView 触发超时处理逻辑**。
...
BE-->>S : 返回关键字和AI首句
S-->>S : 更新状态 : isPlaying=true, history.push(AI首句), 启动倒计时
S-->>GV : 路由跳转至 /game
GV->>TD : 显示倒计时
U->>GV : 输入诗句并提交
...
alt 验证通过
BE-->>S : valid=true, matchType
S-->>S : history.push(玩家诗句), stats.correct++, 记录响应时间, 模糊匹配等统计
S->>S : 调用 aiTurn()
S->>BE : POST /api/v1/game/ai-turn
BE-->>S : 返回AI诗句
S-->>S : history.push(AI诗句), currentRound++, 启动倒计时
else 验证失败
BE-->>S : valid=false
S-->>S : remainingChances--, stats.wrong++
end
TD->>GV : 倒计时归零
GV->>S : 调用 handleTimeout()
S-->>S : remainingChances--, stats.timeoutCount++, stats.wrong++
```
...
- [TimerDisplay.vue](file://frontend/src/components/TimerDisplay.vue#L50-L75)

## 详细组件分析

### 游戏状态与变量映射
- 未开始：isPlaying=false，显示 HomeView，用户可选择关键字。
- 进行中：isPlaying=true，显示 GameView，根据 currentRound、remainingChances 和 history 渲染游戏状态。**倒计时组件根据 timerActive 和 timeRemaining 显示动态倒计时**。
...
进行中 --> 结束 : "handleTimeout() 超时且 remainingChances=0"
进行中 --> 进行中 : "verifyUserSentence() 成功 -> AI 回合"
进行中 --> 进行中 : "handleTimeout() 超时但 remainingChances>0"
```

**图表来源**
- [game.ts](file://frontend/src/stores/game.ts#L34-L204)
- [GameView.vue](file://frontend/src/views/GameView.vue#L14-L194)
...
- 关键点：
  ...
  - **重置所有统计字段（包括新增的 timeoutCount, fuzzyMatchCount, averageResponseTime, fastestResponse, perfectRounds）**
  - **启动倒计时（调用 startTimer()）**
  - 失败时抛出错误
...
- 关键点：
  ...
  - 验证通过：
    - 停止倒计时（stopTimer）
    - **计算响应时间（responseTime）**
    - 将玩家诗句加入 history，correct 和 totalRounds 统计加一
    - **如果 matchType 为 homophone 或 fuzzy，则 fuzzyMatchCount 加一**
    - **更新 averageResponseTime 和 fastestResponse 统计**
    - **如果 currentHintLevel=0 且 remainingChances=3，则 perfectRounds 加一（完美回合）**
    - 重置 remainingChances 和 currentHintLevel
    - 然后调用 aiTurn()
...
- 关键点：
  - 停止倒计时（stopTimer）
  - remainingChances 减一
  - **timeoutCount 和 wrong 统计各加一**
  - 如果 remainingChances <= 0，则调用 endGame() 结束游戏

**章节来源**
- [game.ts](file://frontend/src/stores/game.ts#L80-L86)
- [TimerDisplay.vue](file://frontend/src/components/TimerDisplay.vue#L87-L90)
...
- UI 提示信息：
  ...
  - **TimerDisplay 组件根据 timeRemaining 显示视觉提示（如“时间不多了！”）和动画效果（闪烁、跳动）**
...
- [TimerDisplay.vue](file://frontend/src/components/TimerDisplay.vue#L116-L121)
...
- **TimerDisplay 组件通过 props 和事件与 GameView 通信，实现倒计时的显示和超时处理**

**章节来源**
- [GameView.vue](file://frontend/src/views/GameView.vue#L78-L194)
- [ResultView.vue](file://frontend/src/views/ResultView.vue#L52-L190)
...
- 文件间关系：
  ...
  - 后端 API 提供游戏逻辑支持
  - **GameView 引入 TimerDisplay 组件，TimerDisplay 通过 useGameStore() 访问 store 状态**
...
Result --> Store
Store --> API["后端API"]
Game --> Timer["components/TimerDisplay.vue"]
Timer --> Store
```
...
- **倒计时性能**：
  - TimerDisplay 使用 setInterval 每100ms更新一次，频率较高，但因仅更新 store 中的一个数值，性能影响较小。
  - 动画效果使用 CSS3 实现，性能良好。
...
- **倒计时不工作**：
  - 检查 GameView 是否正确引入了 TimerDisplay 组件
  - 检查 store 的 timerActive 状态是否在 startGame 和 aiTurn 后被正确设置为 true
  - 检查 TimerDisplay 组件的 onMounted 和 watch 是否正常执行
- **超时未处理**：
  - 检查 GameView 是否正确监听了 TimerDisplay 的 timeout 事件
  - 检查 handleTimeout 函数是否被正确调用

**章节来源**
- [game.ts](file://frontend/src/stores/game.ts#L34-L218)
- [GameView.vue](file://frontend/src/views/GameView.vue#L23-L47)
- [TimerDisplay.vue](file://frontend/src/components/TimerDisplay.vue#L70-L74)

## 结论
本项目已成功将游戏状态从分散的组件中迁移至 Pinia store (game.ts) 进行集中管理。通过定义清晰的 GameState 接口和 actions，实现了状态的统一维护和变更。startGame、verifyUserSentence、aiTurn 等 action 分别承担初始化、验证与回合推进职责，history 与 usedPoems 协助维护交互历史与去重校验，stats 提供了全面的游戏数据统计。**本次更新通过引入 TimerDisplay 组件和扩展 stats 接口，成功实现了倒计时机制和精细化的数据统计功能，包括超时、模糊匹配、响应时间、完美回合等，极大地丰富了游戏体验和数据分析维度**。这种集中式状态管理提升了代码的可维护性、可测试性和可扩展性，为未来添加新功能（如成就系统、排行榜）奠定了良好基础。
...
G --> |handleTimeout()| T["超时处理"]
T --> |remainingChances > 0| G
T --> |remainingChances = 0| E
```
...
- 实现了状态的集中化、可预测和可调试
- **新增倒计时状态（timeRemaining, timerActive, roundStartTime）和配置（timerDuration）**
- **扩展 GameStats 接口，新增 timeoutCount, fuzzyMatchCount, averageResponseTime, fastestResponse, perfectRounds 字段**
- **通过 TimerDisplay 组件实现倒计时的UI展示和交互，与 store 状态解耦**

**章节来源**
- [game.ts](file://frontend/src/stores/game.ts#L6-L218)
- [types/game.ts](file://frontend/src/types/game.ts#L18-L30)
