...
AI选择逻辑已优化，主要变更如下：
1. 查询所有包含令字的诗词
2. 使用Prisma的NOT条件排除已使用的诗句（避免逐条判断）
3. 若无可用诗句，返回404
4. 否则随机选择一条可用诗词
5. 将该诗词按多种标点符号（，。！？；）拆分为句子片段，筛选包含令字的片段作为最终返回值；若无匹配片段，则返回整句

```mermaid
flowchart TD
Start(["进入 /api/v1/game/ai-turn"]) --> Parse["解析请求体 {char, usedPoems}"]
Parse --> Query["查询包含令字的所有诗词"]
Query --> Filter["使用NOT条件排除已使用诗句"]
...
- 过滤成本
  - 使用Prisma的NOT条件在数据库层排除已用诗句，显著降低内存过滤开销
- 随机选择
...
## 结论
POST /api/v1/game/ai-turn接口实现了人机对战中的AI出句能力。当前实现已优化为使用NOT条件排除已用诗句，并支持多种标点符号分割，提升查询效率与准确性。满足基本游戏连续性需求。未来可扩展为基于难度、作者偏好或上下文语义的智能选择策略，以提升游戏体验与挑战性。
